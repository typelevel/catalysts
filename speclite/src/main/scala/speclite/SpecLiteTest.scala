...
object SpecLiteTest {
  ...
  private[speclite] lazy val cmdLineParser = new CmdLineParser {
  ...
  def check(params: Parameters, p: Prop): Result = {
    ...
    if (workers > 1) assert(false, "More than one worker is not supported")
    val r =  workerFun(0)
...
private[speclite] trait CmdLineParser {
